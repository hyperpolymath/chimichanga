...
All changes are version controlled. Use RVC (Robot Vacuum Cleaner) for
automated tidying that preserves history.
...
[arabic]
. *Use fuel limits*: Always set reasonable fuel bounds
. *Capture failures*: Save forensic dumps for analysis
. *Version control*: Commit before major changes
. *Test in isolation*: Use separate instances for testing
...
[arabic]
. *Execution hung*: Fuel will eventually exhaust
. *Memory issue*: Each execution is isolated
. *Bad WASM*: Compilation fails safely
. *Trap occurred*: Forensics captured, nothing corrupted

=== See Also

* ARCHITECTURE.md - System design
* docs/capability_model.md - Formal model
